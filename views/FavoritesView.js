      ...
      if (e.target.closest('#exportBtn')) {
        ...
        if (e.target.closest('#exportImgBtn')) {
          ...
          try {
            ...
            const filename = `aeropedia-favoritos-${new Date().toISOString().slice(0,10)}.png`;
            // Try Web Share API with file (modern mobile browsers)
            if (navigator.share && navigator.canShare) {
              try {
                ...
                if (navigator.canShare({ files: [file] })) {
                  await navigator.share({ files: [file], title: 'Mis Favoritos — AeroPedia' });
